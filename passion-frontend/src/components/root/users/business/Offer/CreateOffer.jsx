  ...
  const submitHandler = (event) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      const uploaders = filesToUpload.map(async (file) => {
       ...
       //  formData.append("tags", `dos9ne5qo, passion`);
        formData.append("upload_preset", "Upload-it");
         formData.append("timestamp", (Date.now() / 1000) | 0);

        return await axios
          .post(
            `https://api.cloudinary.com/v1_1/dos9ne5qo/image/upload`,
            formData
            // ,
            // {
      ...
      //   console.log(public_ids[0]);
      //   console.log(img_urls[1]);
      //   console.log(public_ids[1]); 
      //  console.log(img_urls[2]);
      //  console.log(public_ids[3]); 
        setInfo((prevState) => ({
          ...prevState,
    ...
    <>
      <main className={styles.offer_main}>
        ...
        <form className={styles.form_offer} onSubmit={submitHandler}>
          <OfferInputField
            type="text"
            placeholder="ex: Drawing, Music classes etc.."
            label="Hobbie Name"
            name="name"
            error={errors.name}
